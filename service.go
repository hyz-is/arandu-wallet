...
func (s *WalletService) Create(ctx context.Context, actor security.Subject, in CreateRequest) (*Wallet, error) {
	if errs := in.Validate(); errs.Any() {
		return nil, errs
	}

	proposed := Wallet{Name: in.Name}
...
// run at all.
func (s *WalletService) List(ctx context.Context, actor security.Subject, q data.Query) ([]*Wallet, error) {
	g, err := security.Authorize(ctx, s.policy, actor, WalletList, Wallet{})
	if err != nil {
		return nil, err
	}

	column, ok := sortableWallet[q.Sort]
	if !ok {
		return nil, fmt.Errorf("wallet: sort field not allowed: %q", q.Sort)
	}
