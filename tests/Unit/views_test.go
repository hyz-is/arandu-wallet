...
func module(t *testing.T) *wallet.Module {
	t.Helper()
...
func TestThePublicationLandsWhereTheModuleSaysItDoes(t *testing.T) {
	...
	slices.Sort(targets)
	if want := wallet.PublishedPaths(); !slices.Equal(targets, want) {
...
func TestTheModuleRefusesToBootUntilItsViewsAreLinked(t *testing.T) {
	...
	if !strings.Contains(err.Error(), wallet.PublishCommand) {
		t.Errorf("the refusal does not say how to publish the views: %v", err)
	}
	for _, pkg := range wallet.ViewPackages() {
