...
func TestTheApplicationIntegrationHasOneCompositionFile(t *testing.T) {
	...
	guides := []string{
		"README.md",
		".agents/skills/wallet-package/SKILL.md",
	}
	...
	for _, guide := range guides {
		...
		text := string(body)
		for _, required := range []string{"bootstrap/app.go", "wallet.New(", "k.Register"} {
...
func TestThePackageUsesTheModelFirstDataPath(t *testing.T) {
	...
	wants := map[string][]string{
		...
		"service.go": {
			"db     *data.DB",
			"func NewWalletService(db *data.DB) *WalletService",
			"Wallets(s.db)",
			") (*Wallet, error)",
			") ([]*Wallet, error)",
		},
...
func TestTheGuidesTeachTheModelFirstBoundary(t *testing.T) {
	...
	guides := map[string][]string{
		"AGENTS.md":                              {"Wallets(db)", "security.Authorize"},
		"README.md":                              {"Wallets(db)", "Model terminal"},
		"CONTRIBUTING.md":                        {"Wallets(db)", "authorizes before reaching the Model"},
		"SECURITY.md":                            {"Model terminal", "data.Tenant(g)"},
		".agents/skills/README.md":               {"CRUD Repository", "configured Model"},
		".agents/skills/wallet-module/SKILL.md":  {"func Wallets", "security.Authorize"},
		".agents/skills/wallet-policy/SKILL.md":  {"security.Authorize", "Wallets("},
		".agents/skills/wallet-release/SKILL.md": {"Model-first", "configured copy"},
	}
	for path, required := range guides {
		...
		for _, stale := range []string{"WalletRepository", "NewWalletRepository", "repository.go  data access"} {
