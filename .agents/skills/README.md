...
| skill | when it fires |
| --- | --- |
| `wallet-policy` | opening an action, adding an authorized Model-backed use case, or anything answering 403 |
| `wallet-module` | adding a route, a handler, a config field, a response field or a migration |
| `wallet-release` | the gates, the manifest, a dependency, a version, a tag |
| `wallet-vault-notes` | writing the note, the gap or the journal entry, when this checkout sits inside the Arandu Obsidian vault |
| `wallet-package` | installing and wiring this package **into an application** |
