...
4. Only then does a `security.Grant` exist, and `service.go` reaches
   `Wallets(s.db)` and a Model terminal with it.
...
`Wallets(s.db)` call is therefore mandatory even though the terminal itself
also takes `g`.
...
`WalletService` owns `db *data.DB`. Validate first, build the value the Policy
must see, call `security.Authorize`, and only then call `Wallets(s.db)`.
...
```go
	...
	_, err = security.Authorize(ctx, s.policy, actor, WalletView, *record)
```
