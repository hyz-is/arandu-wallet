...
```go
	r.Action(stdhttp.MethodDelete, m.cfg.Prefix+"/{id}", m.destroy).Name("wallet.destroy")
```
...
database or called `Wallets` would bypass the only place the Policy is
guaranteed to run. Read `wallet-policy` before writing the Service method.
...
Service results stay as `*Wallet`/`[]*Wallet`, because copying an embedded
