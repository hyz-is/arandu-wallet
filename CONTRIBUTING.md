...
2. **Every Service method authorizes before reaching the Model.** The Grant is
   then required by the Model terminal; constructing `Wallets(db)` first is
...
CRUD stays on `Wallets(db)` and its Builder. Add a Repository only for a
...
Model-backed entities stay pointers. Copying `Wallet` also copies an embedded
...
licence, the same as the rest of github.com/hyz-is/arandu-wallet.
